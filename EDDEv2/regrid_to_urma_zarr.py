# %%
"""Regrid EDDEv2 Zarr to URMA HR/LR Zarr stores.
...
# Reference paths match EDDEv2/process_and_write_to_zarr.py
OUTPUT_ROOT = "/network/rit/lab/basulab/Projects/DFS/DATA/EDDEv2_NYS/hourly/WRF-MPI"
...
def init_zarr_store(
    ...
    global_title: str = "EDDEv2 regridded to URMA grid",
...
def regrid_and_write_month(
    ...
    if to_hr:
        out = regridders.edde_to_urma_hr(data)
    else:
        out = regridders.edde_to_urma_lr_intended(data)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Regrid EDDEv2 Zarr to URMA HR/LR grids."
    )
    parser.add_argument(
        "--run-type",
        type=str,
        default="SSP2-4.5" if is_interactive() else None,
        choices=sorted(FULL_YEARS_RANGE.keys()),
        help="EDDEv2 run type",
    )
    parser.add_argument(
        "--var_name",
        type=str,
    ...
    parser.add_argument(
        "--start-yearmonth",
        type=str,
        default="202501" if is_interactive() else None,
        help="Start year-month in YYYYMM (e.g., 202501)",
    )
    parser.add_argument(
        "--end-yearmonth",
        type=str,
        default="202501" if is_interactive() else None,
        help="End year-month in YYYYMM (e.g., 202501)",
    )
    parser.add_argument("--full-start-year", type=int, default=None)
    parser.add_argument("--full-end-year", type=int, default=None)
    parser.add_argument(
        "--method",
        type=str,
    ...
    default_start, default_end = FULL_YEARS_RANGE[args.run_type]
    full_start_year = args.full_start_year if args.full_start_year is not None else default_start
    full_end_year = args.full_end_year if args.full_end_year is not None else default_end

    full_times = pd.date_range(
        f"{full_start_year}-01-01T00",
        f"{full_end_year}-12-31T23",
        freq="h",
    )

    input_zarr = f"{OUTPUT_ROOT}/{args.run_type}.zarr"
    if intended_lr is None:
        output_zarr = (
            f"{OUTPUT_ROOT}/{args.run_type}_to_URMA_HR_{method}.zarr"
        )
        target_orog = regridders.urma
        to_hr = True
    else:
        output_zarr = (
            f"{OUTPUT_ROOT}/{args.run_type}_to_{intended_lr}_LR_{method}.zarr"
        )
    ...
    src_ds = xr.open_zarr(input_zarr, consolidated=True)
