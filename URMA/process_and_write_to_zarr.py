...
def init_zarr_store(zarr_store, dates, var_name, orog_path,time_chunk, x_chunk, y_chunk,
                    mode="w", write_global_attrs=False):
...
def ensure_initialized(zarr_store, full_dates, var_name,
                       orog_path,time_chunk, x_chunk, y_chunk):
    """
...
def process_and_write_single_day(date, var_name, zarr_store, dates, full_dates, time_chunk, x_chunk, y_chunk):
    """
    ...
    try:
        ...
        # Use full_dates so writes land in the correct slots of the global time axis
...
if __name__ == "__main__":
    ...
    ensure_initialized(zarr_store, full_dates, var_name, orog_path,time_chunk, x_chunk, y_chunk)
    ...
    for i in tqdm(range(0, len(yyyymmdd), batch_size), desc=f"{var_name} {YEAR}"):
        ...
        Parallel(n_jobs=cpus, backend="loky", verbose=0)(
            delayed(process_and_write_single_day)(date, var_name, zarr_store, dates, full_dates, time_chunk, x_chunk, y_chunk)
